...
function Maps() {
  ...
  const renderTableRows = (data) => {
    return data.map((row, index) => (
      <TableRow key={index}>
        {/* Assuming MLResults is an array of objects, map through the properties */}
  ...
  function fetchMLTypeInfo(mlType, features) {
    return fetch(`https://home-sphere.ca/api/api/${mlType}_info`, {
      ...
      .then((response) => {
        if (!response.ok) {
          throw new Error("Please Run the Algorithm To View Your Results");
        }
        ...
        <Paper>
          <Table>
            ...
            <TableBody>
              ...
              {MLResults ? renderTableRows(MLResults) : null}
            </TableBody>
          </Table>
        </Paper>
      </Modal>
